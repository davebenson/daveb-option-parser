...
OptionParser.prototype.getUsage = function(options)
{
  ...
  this.order.forEach(function(argJName) {
    ...
    if (v.length > maxLeftColumnSize)
      maxLeftColumnSize = v.length;
  });

  var optSize = Math.min(maxLeftColumnSize, optMaxSize);
  var descWidth = width - optIndent - optSize - optSep;

  for (var i = 0; i < this.order.length; i++) {
    var argJName = this.order[i];
    var argInfo = this.jnameToArgInfo[argJName];
    var left = optionLeftColumn[i];
    var dlines = wordwrap(argInfo.description, descWidth);
    var dline_i;
    if (left.length > optSize || dlines.length === 0) {
      lines.push(spaces(optIndent) + left);
      dline_i = 0;
    } else {
      lines.push(spaces(optIndent) +
                 left +
		 spaces(optSize - left.length + optSep) +
		 dlines[0]);
      dline_i = 1;
    }
  }
  for (   ; dline_i < dlines.length; dline_i++)
    lines.push(spcaes(optIndent + optSize + optSep) + dlines[dline_i]);
  lines.push('');
  return lines.join('\n');
};
