...
var builtinTypes = {
  ...
  noarg_callback: new TypeInfo('noarg_callback', function (value, values, argInfo, typeInfo, optionParser) {
    return argInfo.callback(values, argInfo, typeInfo, optionParser);
  }).setRequiresArg(false),
  arg_callback: new TypeInfo('arg_callback', function (value, values, argInfo, typeInfo, optionParser) {
    return argInfo.callback(value, values, argInfo, typeInfo, optionParser);
  }),
  preset: new TypeInfo('preset', function (value, values, argInfo, typeInfo, optionParser) {
    for (var k in argInfo.settings) {
...
function ArgInfo(name, type, description, optionParser) 
{
  this.name = name;
  this.type = type;
...
ArgInfo.prototype.addShortCode = function(code) {
...
OptionParser.prototype.addGeneric = function(name, type, description)
{
  ...
  var info = new ArgInfo(name, type, description, this);
  info._requiresArg = t._requiresArg;
...
OptionParser.prototype.addArgCallback = function(name, label, description, fct) {
  var rv = this.addGeneric(name, 'arg_callback', description).setLabel(label);
  rv.callback = fct;
  return rv;
};

OptionParser.prototype.addNoArgCallback = function(name, description, fct) {
  var rv = this.addGeneric(name, 'noarg_callback', description);
...
OptionParser.prototype.addPreset = function(name, description, settings) {
  var rv = this.addGeneric(name, 'preset', description);
  rv._requiresArg = false;
...
OptionParser.prototype.getUsage = function(options)
{
  ...
  var maybeBasename = function(p) {
    return path.basename(p);
  };

  var programName = self.programName ||
...
OptionParser.prototype.parse = function(args)
{
  ...
  var exclusivesSatisfied = [];
  this.exclusives.forEach(function() { exclusivesSatisfied.push(null); });
  this.arguments = [];
  this.values = {};
  ...
  var handleExclusivesByArgInfo = function(argInfo) {
    for (var ei = 0; ei < argInfo._exclusiveIndices.length; ei++) {
      var curArgInfo = exclusivesSatisfied[argInfo._exclusiveIndices[ei]];
      if (curArgInfo !== null && curArgInfo !== argInfo) {
        optionParser.errorHandler('--' + argInfo.name + ' conflicts with --' + curArgInfo.name);
        return false;
      }
  ...
  for (var i = 2; i < args.length; i++) {
    ...
    if (arg === '--') {
	  ...
	  if (name === 'help-all') {
	    console.log(this.getUsage({showHidden:true}));
            this.modeOrder.forEach(function (mode) {
              var subop = optionParser.modes[mode].parser;
              console.log('\n--- Options for ' + mode + ' ---');
	  ...
	  if (name.substr(0,5) === 'help-') {
        ...
        if (!preparsed) {
          ...
          try {
            arg_value = parseFunc(arg_value, this.values, argInfo, typeInfo);
          } catch (e) {
	...
	argInfos.forEach(function (tuple) {
	  ...
	  if (typeInfo._requiresArg) {
	    ++i;
	    try {
	      optionParser.values[jname] = typeInfo._parseFunc(args[i], optionParser.values, argInfo, typeInfo);
	    } catch (e) {
	    ...
	    try {
	      optionParser.values[jname] = typeInfo._parseFunc(null, optionParser.values, argInfo, typeInfo);
	    } catch (e) {
  ...
  for (var ei = 0; ei < this.exclusives.length; ei++) {
    var exInfo = this.exclusives[ei];
...
OptionParser.prototype.setExclusive = function (required, optionNames) {
  var argInfos = [];
  var optionParser = this;
  var exclusiveIndex = this.exclusives.length;
  optionNames.forEach(function (opName) {
    var jname = optionParser.optionNameToJavascriptName(opName);
    var argInfo = optionParser.jnameToArgInfo[jname];
    if (!argInfo) {
