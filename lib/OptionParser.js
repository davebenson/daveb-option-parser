...
function OptionParser(opts)
{
  ...
  function initWithDef(name, def) {
    this[name] = options[name] === undefined ? def : options[name];
  }
...
OptionParser.prototype.registerType = function(type, parseFunc)
{
  ...
  this.types[type] = new TypeInfo(type, parseFunc);
}
...
OptionParser.prototype.getUsage = function()
{
  ...
  for (var i = 0; i < this.order.length; i++) {
    var arg_info = this.nnameToArgInfo[this.order[i]];
    var l = '  --' + arg_info._name;
    ...
    if (l.length < 20) {
      while (l.length < 20)
        l += ' ';
      lines.push(l + ' ' + arg_info._description);
    } else {
      lines.push(l);
      lines.push('                     ' + arg_info._description);
    }
  }
  lines.push('');
  return lines.join('\n');
...
OptionParser.prototype.parse = function(args)
{
  ...
  for (var arg in this.nnameToArgInfo) {
    if (this.nnameToArgInfo[arg]._mandatory && !(arg in this.values)) {
      this.errorHandler('missing mandatory argument --' + this.nnameToArgInfo[arg]._name);
      return null;
    }
  }
  return this.values;
