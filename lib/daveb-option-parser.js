...
function formatTwoColumn(leftIndent, leftColumnMax,
  ...
  for (const i = 0; i < leftColumn.length; i++) {
...
export class OptionParser
{
  ...
  getUsage(options) {
    ...
    const width = opt.width || 80;
    const optIndent = 2;
    const optMaxSize = opt.optionMaxSize || Math.ceil(width/5);
    const optSep = opt.optionDescriptionSep || 2;
    const showHidden = opt.showHidden === undefined ? false : opt.showHidden;
    const showNonhidden = opt.showNonhidden === undefined ? true : opt.showNonhidden;
    ...
    if (self.modes) {
      ...
      const modeDescriptions = self.modeOrder.map(function(m) {
    ...
    this.order.forEach(function(argJName) {
      const argInfo = self.jnameToArgInfo[argJName];
      const v = '--' + argInfo.name;
      if (argInfo._hidden) {
        if (!showHidden)
          return;
      } else {
      ...
      const typeInfo = builtinTypes[argInfo.type] || self.types[argInfo.type];
  ...
  parse(args = process.arggv) {
    ...
    const handleExclusivesByArgInfo = function(argInfo) {
    ...
    for (let i = 2; i < args.length; i++) {
      ...
      if (arg === '--') {
        ...
        if (arg[1] === '-') {
          ...
          const typeInfo = this.types[type] || builtinTypes[type];
          if (!typeInfo) {
          ...
          if (!preparsed) {
            const parseFunc = typeInfo._parseFunc;
            try {
          ...
          for (let shortIndex = 1; shortIndex < arg.length; shortIndex++) {
            ...
            if (argInfo !== undefined) {
              ...
              const type = argInfo.type;
              const typeInfo = builtinTypes[type] || this.types[type];
          ...
          argInfos.forEach(([shortCode, argInfo, typeInfo, jname]) => {
            if (typeInfo._requiresArg) {
              ++i;
              try {
                this.values[jname] = typeInfo._parseFunc(args[i], this.values, argInfo, typeInfo);
              } catch (e) {
              ...
              try {
                this.values[jname] = typeInfo._parseFunc(null, this.values, argInfo, typeInfo);
              } catch (e) {
