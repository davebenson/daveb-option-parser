...
export class TypeInfoEnum extends TypeInfo {
  constructor(name, values, normalizer) {
    ...
    this.values = Array.isArray(values) ? values : Object.keys(values);
    if (!normalizer) {
      for (const norm of enumNormalizers) {
        if (isNormalized(norm, this.values)) {
          normalizer = norm;
          break;
        }
      }
    ...
    assert.equal(this.valueSet.size, this.values.length);
  }
  parse(value) {
...
function normalizeUsagePrefixOrSuffix(val) {
  ...
  if (typeof(val) === 'string')
    return [val];
  throw new Error(`bad type ${typeof(val)} for usagePrefix or usageSuffix`);
}
...
export class OptionParser
{
  constructor(options = {}) {
    ...
    this.exclusives = [];
    this.usagePrefix = normalizeUsagePrefixOrSuffix(usagePrefix);
    this.usageSuffix = normalizeUsagePrefixOrSuffix(usageSuffix);
  ...
  addFlag(name, description, argOptions) {
    return this.addGeneric(name, 'flag', description, argOptions).setTolerateRepeated();
  }
  ...
  getUsage(options = {}) {
    ...
    for (const line of this.usagePrefix) {
      for (const wline of wordwrap(line, width)) {
        lines.push(wline);
      }
    }
    ...
    for (const line of this.usageSuffix) {
      for (const wline of wordwrap(line, width)) {
        lines.push(wline);
      }
    }
  ...
  _handleExclusives(argInfo, exclusivesSatisfied) {
  ...
  parse(args = process.argv) {
    const encounteredArgInfoNames = new Set();
    ...
    for (let i = 2; i < args.length; i++) {
      ...
      if (arg === '--') {
        ...
        break;
      } else if (arg[0] === '-' && arg !== '-') {               // a single '-' is treated like a non-option argument
        if (arg[1] === '-') {
          // --------------------------------- long option -----------------------------
          let name, arg_value;
          let preparsed = false;
          ...
          if (!(jname in this.jnameToArgInfo)) {
            ...
            if (name.startsWith('help')) {
              return this._handleHelpOptions(name);
            }
          ...
          if (!this._handleExclusives(argInfo, exclusivesSatisfied))
            return null;

          //
          // Parse argument from string (or null for no-arg callback).
          //
          if (!preparsed) {
            try {
          ...
          for (let shortIndex = 1; shortIndex < arg.length; shortIndex++) {
            ...
            if (argInfo !== undefined) {
              if (!argInfo.repeated && !argInfo.tolerateRepeated && encounteredArgInfoNames.has(argInfo.name)) {
                this.errorHandler(`got argument ${argInfo.name} multiple times`);
                return null;
              }
              encounteredArgInfoNames.add(argInfo.name);
    ...
    //
    // If permittedArgs is an array, then we expect exactly that number of arguments.
    //
    ...
    //
    // Verify mandatory exclusive arguments are fulfilled.
    //
