...
function findOptionNameToJavascriptName(on2jn, camelCase, parOP) {
  if (typeof(on2jn) === 'function') {
    ...
    return convert;
  } else if (parOP?.optionNameToJavascriptName) {
    return parOP?.optionNameToJavascriptName;
  } else {
...
export class OptionParser
{
  ...
  addGeneric(name, type, description, argOptions) {
    ...
    if (jname in this.jnameToArgInfo) {
      throw new Error(`adding ${name} ${type}: already have a flag with the same target`);
    }
  ...
  addMode(mode,
          {modeJavascriptName, ...optionParserInitOptions},
    ...
    const subOptionParser = (optionParserInitOptions instanceof OptionParser) 
      ? // If given an OptionParser, simply
        ...
        ((op) => {
          ...
          op.parent = this;
          return op;
        })(optionParserInitOptions)
      : // otherwise, create a new OptionParser with
        // the given options, and parent=this.
        new OptionParser({parent:this, ...optionParserInitOptions});
  ...
  addModeAlias(newMode, origMode) {
    if (!(origMode in this.modes)) {
      throw new Error(`original mode ${origMode} not available`);
    }
    if (newMode in this.modes) {
      throw new Error(`new mode ${origMode} already registered`);
    }
    this.modeAliases ??= {};
    if (newMode in this.modeAliases) {
      throw new Error(`new mode ${origMode} already registered as alias`);
    }
  ...
  dumpModeTree(indent) {
    ...
    if (this.modeOrder) {
      for (const mode of this.modeOrder) {
        ...
        console.log(`${ind}${mode}: ${desc}`);
        this.modes[mode].parser.dumpModeTree(indent + 2);
      }
    }
  }
    ...
    for (let i = 2; i < args.length; i++) {
      ...
      if (arg === '--') {
        ...
        if (!modeInfo) {
          ...
          this.errorHandler(`unknown mode ${arg}: expected one of: ${modeStr}`);
          return null;
        }
    ...
    if (Array.isArray(this.permittedArgs)) {
      if (this.values.arguments.length < this.permittedArgs.length) {
        const progName = this.parent
                       ...
                       : args[1];
        throw new Error(`expected ${this.permittedArgs.length} args to ${progName} but only got ${this.values.arguments.length}`);
      }
    }
